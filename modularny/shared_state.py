...
class SharedState:
    ...
    def __init__(self, root):
        ...
        # NOVÉ: Spread Scanner - vlastné bloky a top list
        self.spread_symbol_blocks = {}  # {name: [SYM,...]}
        self.spread_selected_block = tk.StringVar(value="-- Vybrať blok --")
        self.top_spread_symbols = []  # top-N symbols generated by scanner
        ...
        try:
            if os.path.exists(self.settings_file):
                with open(self.settings_file, 'r', encoding='utf-8') as f:
                    ...
                    # Spread scanner storage
                    self.spread_symbol_blocks = data.get('spread_symbol_blocks', {})
                    self.spread_selected_block.set(data.get('spread_selected_block', "-- Vybrať blok --"))
                    self.top_spread_symbols = data.get('top_spread_symbols', [])
            else:
                self.saved_strategies = {}
                ...
                self.symbol_blocks = {}
                self.spread_symbol_blocks = {}
                self.top_spread_symbols = []
    ...
    def save_settings_file(self):
        ...
        try:
            data = {
                ...
                'spread_symbol_blocks': getattr(self, 'spread_symbol_blocks', {}),
                'spread_selected_block': self.spread_selected_block.get() if hasattr(self, 'spread_selected_block') else "-- Vybrať blok --",
                'top_spread_symbols': getattr(self, 'top_spread_symbols', [])
            }
...
def open_symbol_block_manager(state, on_close_callback=None, blocks_attr='symbol_blocks'):
    ...
    def refresh_list():
        ...
        blocks = getattr(state, blocks_attr, {}) or {}
    ...
    def save_block():
        ...
        blocks = getattr(state, blocks_attr, None)
        if blocks is None:
            blocks = {}
            setattr(state, blocks_attr, blocks)
        blocks[name] = symbols
